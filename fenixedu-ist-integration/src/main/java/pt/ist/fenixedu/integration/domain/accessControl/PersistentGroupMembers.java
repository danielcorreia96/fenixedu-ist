...
public class PersistentGroupMembers extends PersistentGroupMembers_Base {
    ...
    private void updateUnitSenders() {
        org.fenixedu.messaging.core.domain.Sender sender = getUnit().getSender();
    ...
    private boolean hasRecipientWithToName(org.fenixedu.messaging.core.domain.Sender sender, final String toName) {
        return sender.getRecipients().stream().filter(r -> r.getPresentationName().equals(toName)).findAny().isPresent();
    }
}
